...
#[test]
fn cmp_drops_datagrams_from_unauthorized_source() {
    use rsx_dxs::config::CmpConfig;
    use rsx_dxs::encode_utils::as_bytes;
    use rsx_dxs::cmp::CmpReceiver;

    // Receiver expects sender at 127.0.0.1:<some_port>.
    // We then bind a *separate* sender on 127.0.0.2 and verify
    // its packets are dropped.
    let recv_sock = UdpSocket::bind("127.0.0.1:0").unwrap();
    let recv_addr = recv_sock.local_addr().unwrap();
    drop(recv_sock);

    // Configure receiver: expect sender from 127.0.0.1
    let expected_sender: SocketAddr =
    ...
    // Build a malicious sender from 127.0.0.2
    // (loopback alias) and send a valid fill record.
    let attacker = match UdpSocket::bind("127.0.0.2:0") {
        Ok(s) => s,
        Err(_) => {
            // 127.0.0.2 not configured on this host; skip.
            // Test still validates the same-IP path below.
            return;
        }
    };
    let fill = fill_payload(1);
    let payload_bytes = as_bytes(&fill);
    ...
    let mut buf = Vec::with_capacity(
        WalHeader::SIZE + payload_bytes.len(),
    );
    buf.extend_from_slice(&hdr.to_bytes());
    buf.extend_from_slice(payload_bytes);
    attacker
        .send_to(&buf, recv_addr)
        .expect("send to recv_addr");

    thread::sleep(Duration::from_millis(50));

    // Receiver should have dropped it: no event delivered.
    let got = receiver.try_recv();
    assert!(
        got.is_none(),
        "receiver must drop datagram from unauthorized source ip; got {got:?}"
    );
}

#[test]
fn cmp_allows_any_source_when_configured() {
    use rsx_dxs::config::CmpConfig;
    use rsx_dxs::encode_utils::as_bytes;
    use rsx_dxs::cmp::CmpReceiver;
    ...
    // With allow_any_source, the datagram is accepted.
    let got = receiver.try_recv();
    assert!(
        got.is_some(),
        "with allow_any_source=true, datagram should be accepted"
    );
}
