...
fn fills_precede_order_done_always() {
    ...
    for i in 0..10u32 {
        let mut sell = make_order(50_000, 10, Side::Sell, 100 + i, 100 + i as u64);
        process_new_order(&mut book, &mut sell);
    }

    let mut buy = make_order(50_000, 100, Side::Buy, 1, 1);
    ...
    for maker_id in 100..110u32 {
        let fill_idx = events.iter().position(|e| match e {
            Event::Fill { maker_user_id, .. } => *maker_user_id == maker_id,
            _ => false,
        });
        let done_idx = events.iter().position(|e| match e {
            Event::OrderDone { user_id, .. } => *user_id == maker_id,
            _ => false,
        });
        assert!(
...
fn exactly_one_completion_per_order() {
    ...
    let mut done_counts = std::collections::HashMap::<u64, u32>::new();
    for e in events {
        if let Event::OrderDone { order_id_lo, .. } = e {
            *done_counts.entry(*order_id_lo).or_default() += 1;
        }
    }

    for (oid, count) in &done_counts {
        assert_eq!(*count, 1, "order {} got {} completions", oid, count,);
    }
}
...
fn fifo_within_price_level_verified() {
    ...
    for (i, &uid) in maker_ids.iter().enumerate() {
        let mut sell = make_order(50_000, 10, Side::Sell, uid, uid as u64);
        sell.timestamp_ns = 1000 + i as u64;
        process_new_order(&mut book, &mut sell);
    }

    // Buy sweeps all 5
    let mut buy = make_order(50_000, 50, Side::Buy, 1, 1);
    process_new_order(&mut book, &mut buy);
    let events = book.events();
...
fn best_bid_ask_coherent_after_every_op() {
    ...
    for (i, (price, side)) in ops.iter().enumerate() {
        let mut order = make_order(*price, 10, *side, i as u32 + 1, i as u64 + 1);
...
fn slab_no_leak_after_1m_operations() {
    let mut book = Orderbook::new(test_config(), 1024, 50_000);

    for i in 0..1_000_000u64 {
        let mut order = make_order(50_000, 10, Side::Buy, 1, i);
        process_new_order(&mut book, &mut order);
        let events = book.events();
    ...
    let mut order = make_order(50_000, 10, Side::Buy, 1, 999_999_999);
    process_new_order(&mut book, &mut order);
    let events = book.events();
    assert!(events
        .iter()
...
fn event_seq_monotonic_within_symbol() {
    ...
    for e in events {
        match e {
            Event::Fill { .. } => {
                assert!(!seen_done, "fill after done",);
                assert!(!seen_bbo, "fill after bbo",);
            }
            Event::OrderDone { .. } => {
                assert!(!seen_bbo, "done after bbo",);
                seen_done = true;
            }
            Event::BBO { .. } => {
                seen_bbo = true;
