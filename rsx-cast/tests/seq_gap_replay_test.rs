...
fn frame(record_type: u16, payload: &[u8]) -> Vec<u8> {
    ...
    let header =
        WalHeader::new(record_type, payload.len() as u16, crc);
...
fn send_heartbeat(
    ...
    probe
        .send_to(
            &frame(RECORD_HEARTBEAT, as_bytes(&hb)),
            dest,
        )
        .unwrap();
}
...
fn seq_gap_faults_then_tcp_replay_resumes() {
    ...
    let (last_delivered, gap_start) = faulted_at
        .expect("receiver should FAULT on the seq=2 gap");
    ...
    match receiver.try_recv() {
        CastRecv::Data(_, p) => {
            ...
            assert_eq!(
                rec.seq, 4,
                "live stream must resume after replay+reset",
            );
        }
        other => panic!(
            "expected Data(seq=4) after reset, got {other:?}"
        ),
    }
}
