...
async fn serve(wal_dir: std::path::PathBuf, addr: SocketAddr, tls: rsx_cast::TlsConfig) {
    let service = ReplicationService::new(wal_dir, tls).unwrap();
...
async fn not_available_falls_through_to_next_endpoint() {
    ...
    let mut writer = WalWriter::new(STREAM_ID, &dir_b, 64 * 1024 * 1024).unwrap();
    for i in 1..=3u64 {
        let mut rec = fill(i);
