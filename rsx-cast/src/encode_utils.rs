...
pub fn encode_record(record_type: u16, payload: &[u8]) -> Vec<u8> {
    let crc32 = compute_crc32(payload);
    let header = WalHeader::new(record_type, payload.len() as u16, crc32);
...
pub fn decode_payload<T: Copy>(payload: &[u8]) -> Option<T> {
    ...
    Some(unsafe { std::ptr::read_unaligned(payload.as_ptr() as *const T) })
}
