...
def drift_check() -> dict:
    ...
    artifact total (total_passed must equal PLAYWRIGHT_CANONICAL=419).

    Playground source canonical: 410 tests across spec files.
    Full release canonical: PLAYWRIGHT_CANONICAL = 419 (includes 9 webui).

    Returns dict with:
      ok: bool — True if playground count matches PLAYGROUND_SPEC_COUNT
      actual: int — counted playground tests
      canonical: int — PLAYGROUND_SPEC_COUNT (410)
      total_canonical: int — PLAYWRIGHT_CANONICAL (419)
      drift: int — actual - canonical (0 = no drift)
      detail: dict[spec_name, int] — per-spec counts
    """
    import re

    PLAYGROUND_SPEC_COUNT = 410
...
def main():
    ...
    # Release gate: all gates green AND playwright == 419/419
    all_green = (
        g1 == "pass"
