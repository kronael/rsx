...
36 hours to full WAL/streaming infrastructure.
...
**rsx-dxs** (1,339 lines) -- WAL + event streaming:
...
- Config from TOML (recorder.toml.example included)
...
- Proto: `proto/dxs.proto` compiled via tonic-build
...
**8,800 lines of spec for 2,800 lines of code.** 3.1:1
...
list. Every edit to any file triggers it. Had to re-add
crates to the members list repeatedly during sessions.
...
**0ns WAL append.** The WAL write is a memcpy, not a disk
operation. Durability is on a 10ms timer. Bounded data loss
is the design -- deterministic replay + client retry + dedup
makes this correct.

**fsync is 1000x more expensive than serialization.** The
wire format debate (protobuf vs FlatBuffers vs raw structs)
is noise. fsync is 200us-2ms. Serialization is 50-200ns.
The only optimization that matters is batching fsync.
...
file. Replay service reads from the beginning. No broker,
no ZooKeeper, no consumer groups. The recorder IS the
replica -- it's a consumer that writes to a different disk.

**gRPC is temporary.** tonic/tokio uses epoll. For a
streaming service at 100K+ records/sec, that's too many
syscalls. Plan: monoio (io_uring) for gateway + market
data from day one, migrate DXS gRPC later. Reference
impl in `../trader/monoio-client/` is production-proven.
...
3. **picking-a-wire-format.md** -- raw structs vs protobuf
   vs FlatBuffers vs Cap'n Proto. Hybrid: FlatBuffers
   external, raw structs internal
...
```
Implementation:     ~2,800 lines across 5 crates
Tests:              ~1,100 lines, 101 tests, all passing
Specifications:     ~8,800 lines across 28 files
Blog:               ~1,400 lines across 4 posts
Docs:               ~16 root-level markdown files
Ratio spec:code:    3.1:1
```
