//! RSX trading terminal — UI state + rendering (testable core).
//!
//! The binary (`main.rs`) owns the terminal + event loop; everything
//! that decides *what to draw* and *how a key mutates state* lives
//! here so it can be driven from tests against a `TestBackend` with no
//! real terminal. Live data is not wired yet — `App::mock` supplies
//! placeholder state; the gateway WS client (webproto 49, reusing
//! rsx-types / rsx-messages) is the next step.
...
impl App {
    ...
    pub fn spread(&self) -> i64 {
        let best_ask = self.asks.last().map(|a| a.0).unwrap_or(0);
...
/// Apply one key to the app. Returns whether the loop should quit.
/// Pure over `app` + `code` so tests can script a session without a
/// terminal.
pub fn handle_key(app: &mut App, code: KeyCode) -> Control {
    match code {
