...
| `wire.rs` | The protobuf frame codec: `OrderReq`↔`WireOrder`, `GwEvent`↔`WireEvent`, over length-delimited frames. |
...
no `protoc`. It is deliberately minimal: exactly the fields the TUI
submits and renders.

- **Client → server:** `WireOrder { side, price, qty, tif }`.
- **Server → client:** `WireEvent`, a `oneof` over the wire event kinds —
  `Book`, `Trade`, `Accepted`, `Fill`, `Done`, `Rejected`, `Position`,
  `Latency`. These mirror `GwEvent` one-to-one.

The `Latency` frame carries the server-stamped `internal` (casting
GW→Risk→ME→Risk→GW) and `engine` (match + risk) legs; the client fills
the `net` (client↔gateway) leg from its own measured round-trip.
...
plus carrying the session's identity (today the JWT the WS handshake
carried) over QUIC and distributing the gateway cert — is a follow-up,
