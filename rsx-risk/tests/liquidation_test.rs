...
fn maybe_process_immediate_first_order() {
    let mut e = make_engine();
    e.enqueue(1, 100, 0);
    let (orders, _losses) = e.maybe_process(0, &|_u, _s| 10, &|_s| 50000);
...
fn maybe_process_respects_delay() {
    ...
    e.enqueue(1, 100, 0);
    // First order at t=1s
    let (_, _) = e.maybe_process(1_000_000_000, &|_, _| 10, &|_| 50000);
    // round is now 2, delay = 2 * 1s = 2s
    // At t=2s (1s after last), too early
    let (orders, _losses) = e.maybe_process(2_000_000_000, &|_, _| 10, &|_| 50000);
...
fn maybe_process_delay_elapsed() {
    ...
    // need now >= 1s + 2s = 3s
    let (orders, _losses) = e.maybe_process(3_000_000_000, &|_, _| 10, &|_| 50000);
    assert_eq!(orders.len(), 1);
}
...
fn maybe_process_escalates_slippage() {
    ...
    // /10000 = 99900
    let (orders, _losses) = e.maybe_process(0, &|_, _| 10, &|_| mark);
    assert_eq!(orders[0].price, 99900);

    // Round 2 (slip=40bps): sell price = 100000*(10000-40)
    // /10000 = 99600
    let (orders, _losses) = e.maybe_process(10_000_000_000, &|_, _| 10, &|_| mark);
    assert_eq!(orders[0].price, 99600);

    // Round 3 (slip=90bps): sell price = 100000*(10000-90)
    // /10000 = 99100
    let (orders, _losses) = e.maybe_process(30_000_000_000, &|_, _| 10, &|_| mark);
...
fn maybe_process_generates_reduce_only_order() {
    let mut e = make_engine();
    e.enqueue(1, 100, 0);
    let (orders, _losses) = e.maybe_process(0, &|_, _| 10, &|_| 50000);
    assert_eq!(orders.len(), 1);
...
fn maybe_process_order_qty_equals_abs_position() {
    let mut e = make_engine();
    e.enqueue(1, 100, 0);
    let (orders, _losses) = e.maybe_process(0, &|_, _| -7, &|_| 50000);
...
fn maybe_process_order_price_with_slippage() {
    ...
    e.enqueue(1, 100, 0);
    let (orders, _losses) = e.maybe_process(0, &|_, _| 10, &|_| mark);
    assert_eq!(orders[0].price, mark * (10_000 - 10) / 10_000);

    // Short -> buy: mark*(10000+slip)/10000
    let mut e2 = make_engine();
    e2.enqueue(2, 100, 0);
    let (orders, _losses) = e2.maybe_process(0, &|_, _| -10, &|_| mark);
    assert_eq!(orders[0].price, mark * (10_000 + 10) / 10_000);
}
...
fn maybe_process_marks_done_after_max_rounds() {
    ...
    for i in 0..4 {
        let (_, _) = e.maybe_process(i as u64, &|_, _| 10, &|_| 50000);
    }
    assert_eq!(e.active[0].status, LiquidationStatus::Done);

    // No more orders
    let (orders, _losses) = e.maybe_process(100, &|_, _| 10, &|_| 50000);
    assert_eq!(orders.len(), 0);
}
...
fn zero_position_no_order() {
    let mut e = make_engine();
    e.enqueue(1, 100, 0);
    let (orders, _losses) = e.maybe_process(0, &|_, _| 0, &|_| 50000);
    assert_eq!(orders.len(), 0);
    // Also marks done
    assert_eq!(e.active[0].status, LiquidationStatus::Done);
}
...
fn multiple_users_independent_rounds() {
    let mut e = LiquidationEngine::new(1_000_000_000, 10, 10, 500);
    e.enqueue(1, 100, 0);
    e.enqueue(2, 100, 500_000_000); // enqueued later

    // Both fire first order (last_order_ns=0)
    let (orders, _losses) =
        e.maybe_process(500_000_000, &|u, _| if u == 1 { 10 } else { -5 }, &|_| {
            50000
        });
    ...
    let (orders, _losses) =
        e.maybe_process(1_500_000_000, &|u, _| if u == 1 { 10 } else { -5 }, &|_| {
            50000
        });
    assert_eq!(orders.len(), 0);
    ...
    let (orders, _losses) =
        e.maybe_process(2_500_000_000, &|u, _| if u == 1 { 10 } else { -5 }, &|_| {
            50000
        });
    assert_eq!(orders.len(), 2);
}
...
fn halt_symbol_skips_liquidation() {
    ...
    e.enqueue(1, 0, 0);
    e.halt_symbol(0);
    let (orders, _) = e.maybe_process(0, &|_, _| 100, &|_| 50000);
...
fn multiple_positions_all_get_orders() {
    ...
    e.enqueue(1, 1, 0);
    e.enqueue(1, 2, 0);
    let (orders, _) = e.maybe_process(0, &|_, _| 10, &|_| 50000);
...
fn partial_fill_reduces_position() {
    ...
    let (orders, _) = e.maybe_process(1, &|_, _| 40, &|_| 50000);
...
fn full_fill_closes_position() {
    ...
    let (orders, _) = e.maybe_process(1, &|_, _| 0, &|_| 50000);
    assert_eq!(orders.len(), 0);
    assert_eq!(e.active[0].status, LiquidationStatus::Done);
}
...
fn frozen_margin_released_on_entry() {
    ...
    let mut e = make_engine();
    e.enqueue(1, 0, 0);
    let (orders, _) = e.maybe_process(0, &|_, _| 50, &|_| 10000);
    assert_eq!(orders.len(), 1);
}
...
fn mark_price_update_rechecks_liquidating_users() {
    ...
    let (orders, _) = e.maybe_process(1, &|_, _| 10, &|_| 40000);
...
fn mark_price_zero_pauses_round_no_increment() {
    ...
    let (orders, _) = e.maybe_process(0, &|_, _| 10, &|_| 0);
    assert!(orders.is_empty());
...
fn multiple_symbols_independent_round_timers() {
    ...
    e.enqueue(1, 1, 500_000_000); // later
                                  // Both fire first order (last_order_ns=0)
    let (orders, _) = e.maybe_process(1_000_000_000, &|_, _| 10, &|_| 50000);
    assert_eq!(orders.len(), 2);
    // Now sym 0 at round 2 (delay=2s from t=1s)
    // sym 1 at round 2 (delay=2s from t=1s)
    // At t=2s: both too early (need t>=3s)
    let (orders, _) = e.maybe_process(2_000_000_000, &|_, _| 10, &|_| 50000);
    assert_eq!(orders.len(), 0);
    // At t=3s: both fire
    let (orders, _) = e.maybe_process(3_000_000_000, &|_, _| 10, &|_| 50000);
    assert_eq!(orders.len(), 2);
}
...
fn rapid_fire_maybe_process_no_duplicate_orders() {
    ...
    let (o1, _) = e.maybe_process(1_000_000_000, &|_, _| 10, &|_| 50000);
    assert_eq!(o1.len(), 1);
    // Immediate second call: delay not elapsed
    // round=2, delay=2s, need t >= 1s+2s = 3s
    let (o2, _) = e.maybe_process(1_000_000_000, &|_, _| 10, &|_| 50000);
    assert_eq!(o2.len(), 0);
    // Still too early at t=2.5s
    let (o3, _) = e.maybe_process(2_500_000_000, &|_, _| 10, &|_| 50000);
    assert_eq!(o3.len(), 0);
}
...
fn socialized_loss_when_round_exceeds_max_rounds() {
    ...
    e.enqueue(1, 0, 0);
    // Round 1: order
    let (orders, losses) = e.maybe_process(0, &|_, _| 10, &|_| 50000);
    assert_eq!(orders.len(), 1);
    assert!(losses.is_empty());
    // Round 2: order
    let (orders, losses) = e.maybe_process(1, &|_, _| 10, &|_| 50000);
    assert_eq!(orders.len(), 1);
    assert!(losses.is_empty());
    // Round 3 > max_rounds=2: socialized loss
    let (orders, losses) = e.maybe_process(2, &|_, _| 10, &|_| 50000);
    assert!(orders.is_empty());
...
fn base_delay_zero_all_rounds_immediate() {
    ...
    for i in 0..5 {
        let (orders, _) = e.maybe_process(0, &|_, _| 10, &|_| 50000);
        assert_eq!(orders.len(), 1, "round {} should fire", i + 1);
    }
