...
fn per_ip_rate_limit_enforced_per_ip() {
    ...
    for _ in 0..100 {
        let limiter = state.ip_limiters.entry(ip1).or_insert_with(per_ip);
    ...
    // IP1 should now be rate limited
    {
        ...
        assert!(
            !limiter.try_consume(),
            "ip1 should be rate limited after 100"
        );
    }

    // IP2 should still have full capacity
    for _ in 0..100 {
        let limiter = state.ip_limiters.entry(ip2).or_insert_with(per_ip);
    ...
    // IP2 should now be rate limited
    {
        ...
        assert!(
            !limiter.try_consume(),
            "ip2 should be rate limited after 100"
        );
    }
}
...
fn per_ip_and_per_user_limits_enforced_independently() {
    ...
    for _ in 0..10 {
        let limiter = state.user_limiters.entry(user_id).or_insert_with(per_user);
    ...
    // User should be rate limited
    {
        ...
        assert!(
            !limiter.try_consume(),
            "user should be rate limited after 10"
        );
    }

    // IP can still make 100 requests (independent of user limit)
    for _ in 0..100 {
        let limiter = state.ip_limiters.entry(ip).or_insert_with(per_ip);
    ...
    // IP should now be rate limited
    {
        ...
        assert!(
            !limiter.try_consume(),
            "ip should be rate limited after 100"
        );
    }
}
...
fn multiple_users_from_same_ip_share_ip_limit() {
    ...
    for _ in 0..10 {
        let limiter = state.user_limiters.entry(user1).or_insert_with(per_user);
        assert!(limiter.try_consume());
    }

    for _ in 0..10 {
        let limiter = state.user_limiters.entry(user2).or_insert_with(per_user);
        assert!(limiter.try_consume());
    }
