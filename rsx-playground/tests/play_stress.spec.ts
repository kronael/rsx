...
// ── stress-down contract ──────────────────────────────────────────────
//
// Strict contract tests: when the stress endpoint fails it MUST return
// HTTP 502 with a structured JSON body containing exactly three fields:
//   code    — machine-readable error identifier (string, non-empty)
//   message — human-readable description       (string, non-empty)
//   context — diagnostic object with gateway_url (string)
//
// No legacy fields (status, error, report_id) may appear.
// These tests use an unreachable gateway (ws://localhost:1) which
// reliably produces GATEWAY_UNREACHABLE on every run.

test.describe("stress-down contract", () => {
  const DOWN_FORM = {
    form: { rate: "10", duration: "1", gateway_url: "ws://localhost:1" },
  };

  test("returns HTTP 502", async ({ request }) => {
    const res = await request.post("/api/stress/run", DOWN_FORM);
    expect(res.status()).toBe(502);
  });
  ...
  test("body.code is a non-empty string", async ({ request }) => {
    const res = await request.post("/api/stress/run", DOWN_FORM);
    expect(res.status()).toBe(502);
    const body = await res.json();
  ...
  test("body.code is GATEWAY_UNREACHABLE for closed port",
    async ({ request }) => {
      const res = await request.post("/api/stress/run", DOWN_FORM);
      expect(res.status()).toBe(502);
      const body = await res.json();
  ...
  test("body.message is a non-empty string", async ({ request }) => {
    const res = await request.post("/api/stress/run", DOWN_FORM);
    expect(res.status()).toBe(502);
    const body = await res.json();
    expect(typeof body.message).toBe("string");
  ...
  test("body.context is a non-null object", async ({ request }) => {
    const res = await request.post("/api/stress/run", DOWN_FORM);
    expect(res.status()).toBe(502);
    const body = await res.json();
    expect(typeof body.context).toBe("object");
  ...
  test("body.context.gateway_url echoes the submitted URL",
    async ({ request }) => {
      const res = await request.post("/api/stress/run", DOWN_FORM);
      expect(res.status()).toBe(502);
      const body = await res.json();
      expect(body.context.gateway_url).toBe("ws://localhost:1");
    }
  );

  test("body has no legacy status field", async ({ request }) => {
    const res = await request.post("/api/stress/run", DOWN_FORM);
    expect(res.status()).toBe(502);
    const body = await res.json();
    expect(body.status).toBeUndefined();
  ...
  test("body has no legacy error field", async ({ request }) => {
    const res = await request.post("/api/stress/run", DOWN_FORM);
    expect(res.status()).toBe(502);
    const body = await res.json();
    expect(body.error).toBeUndefined();
  ...
  test("body has no report_id field on failure", async ({ request }) => {
    const res = await request.post("/api/stress/run", DOWN_FORM);
    expect(res.status()).toBe(502);
    const body = await res.json();
    expect(body.report_id).toBeUndefined();
  ...
  test("all three contract fields present simultaneously",
    async ({ request }) => {
      const res = await request.post("/api/stress/run", DOWN_FORM);
      expect(res.status()).toBe(502);
      const body = await res.json();
      // All three fields must be present in one request (not spread
      // across separate calls which could hit different states).
      expect(Object.keys(body)).toContain("code");
      expect(Object.keys(body)).toContain("message");
      expect(Object.keys(body)).toContain("context");
      expect(typeof body.code).toBe("string");
      expect(typeof body.message).toBe("string");
      expect(typeof body.context).toBe("object");
    }
