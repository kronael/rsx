...
fn roundtrip(book: &Orderbook) -> Box<Orderbook> {
    let mut buf = Vec::new();
...
fn single_order_roundtrip() {
    ...
    book.insert_resting(50_000, 10, Side::Buy, 0, 42, false, 1000, 0, 1);
    let restored = roundtrip(&book);
...
fn multiple_orders_both_sides() {
    let mut book = make_book();
    book.insert_resting(49_990, 5, Side::Buy, 0, 1, false, 100, 0, 10);
    book.insert_resting(49_995, 3, Side::Buy, 0, 2, false, 200, 0, 20);
    book.insert_resting(50_010, 7, Side::Sell, 0, 3, false, 300, 0, 30);
    let restored = roundtrip(&book);
    assert_eq!(restored.sequence, 3);
    assert_ne!(restored.best_bid_tick, NONE);
    assert_ne!(restored.best_ask_tick, NONE);
    assert_eq!(restored.best_bid_tick, book.best_bid_tick,);
    assert_eq!(restored.best_ask_tick, book.best_ask_tick,);
}
...
fn sequence_preserved() {
    let mut book = make_book();
    for i in 0..10 {
        book.insert_resting(49_990 + i, 1, Side::Buy, 0, 1, false, 100, 0, i as u64);
    }
...
fn cancel_then_snapshot_preserves_state() {
    let mut book = make_book();
    let h1 = book.insert_resting(49_990, 5, Side::Buy, 0, 1, false, 100, 0, 1);
    let _h2 = book.insert_resting(49_995, 3, Side::Buy, 0, 2, false, 200, 0, 2);
...
fn fill_then_snapshot_user_positions() {
    ...
    book.insert_resting(50_000, 10, Side::Sell, 0, 2, false, 100, 0, 20);
...
fn snapshot_and_wal_recovery_restores_book() {
    ...
    // Resting bids
    book.insert_resting(49_990, 10, Side::Buy, 0, 1, false, 100, 0, 1);
    book.insert_resting(49_995, 5, Side::Buy, 0, 2, false, 200, 0, 2);
    // Resting asks
    book.insert_resting(50_010, 8, Side::Sell, 0, 3, false, 300, 0, 3);
    book.insert_resting(50_005, 12, Side::Sell, 0, 4, false, 400, 0, 4);
    ...
    // New resting bid
    book.insert_resting(49_998, 20, Side::Buy, 0, 5, false, 600, 0, 6);
    ...
    let mut cursor = std::io::Cursor::new(snap_buf);
    let mut restored = snapshot::load(&mut cursor).unwrap();
    assert_eq!(restored.sequence, snap_seq);
    assert_eq!(restored.best_bid_tick, snap_best_bid,);
    assert_eq!(restored.best_ask_tick, snap_best_ask,);

    // -- Replay post-snapshot WAL events --
    restored.insert_resting(49_998, 20, Side::Buy, 0, 5, false, 600, 0, 6);
    ...
    assert_eq!(restored.best_bid_tick, final_best_bid,);
