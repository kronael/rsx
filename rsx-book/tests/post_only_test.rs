...
fn incoming(price: i64, qty: i64, side: Side, post_only: bool) -> IncomingOrder {
    IncomingOrder {
        price,
        qty,
        remaining_qty: qty,
...
fn post_only_buy_at_best_ask_cancelled() {
    ...
    let mut order = incoming(50_100, 100, Side::Buy, true);
...
fn post_only_buy_above_best_ask_cancelled() {
    let mut book = test_book();
    book.insert_resting(50_100, 100, Side::Sell, 0, 1, false, 0, 0, 0);
    let mut order = incoming(50_200, 100, Side::Buy, true);
    process_new_order(&mut book, &mut order);
...
fn post_only_buy_below_best_ask_inserted() {
    let mut book = test_book();
    book.insert_resting(50_100, 100, Side::Sell, 0, 1, false, 0, 0, 0);
    let mut order = incoming(50_099, 100, Side::Buy, true);
    process_new_order(&mut book, &mut order);
    ...
    assert!(!book
        .events()
        .iter()
        .any(|e| matches!(e, Event::OrderCancelled { .. })));
}
...
fn post_only_sell_at_best_bid_cancelled() {
    ...
    let mut order = incoming(49_900, 100, Side::Sell, true);
    process_new_order(&mut book, &mut order);
...
fn post_only_sell_below_best_bid_inserted() {
    let mut book = test_book();
    book.insert_resting(49_900, 100, Side::Buy, 0, 1, false, 0, 0, 0);
    let mut order = incoming(49_901, 100, Side::Sell, true);
    process_new_order(&mut book, &mut order);
...
fn post_only_on_empty_book_inserted() {
    let mut book = test_book();
    let mut order = incoming(50_000, 100, Side::Buy, true);
    process_new_order(&mut book, &mut order);
...
fn non_post_only_crosses_fills_normally() {
    let mut book = test_book();
    book.insert_resting(50_100, 100, Side::Sell, 0, 1, false, 0, 0, 0);
    let mut order = incoming(50_100, 100, Side::Buy, false);
    process_new_order(&mut book, &mut order);
