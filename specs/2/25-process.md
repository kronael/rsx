...
**links** (inter-process casting/UDP or WAL/TCP connections).
...
- **Link**: inter-process transport (casting/UDP hot path, WAL/TCP cold path).
...
- Some auxiliary processes may be **tokio-only** end-to-end
  (no monoio), while still reusing common casting/WAL/telemetry
  components.

## Inter-Process Links

- **casting/UDP (hot path)**: one WAL record per datagram.
...
Links:
- casting/UDP to Risk (orders, responses).

### Risk Process

Tiles:
- Core Tile (busy-spin): margin checks, position updates.
- Network Tile (monoio or raw UDP): casting ingress/egress.
- Persistence Tile (tokio): Postgres write-behind.

Links:
- casting/UDP from Gateway (orders).
- casting/UDP to Matching (validated orders).
- casting/UDP from Matching (fills/dones).
...
- Replay/Streamer Tile (tokio): WAL/TCP replication streaming.

Links:
- casting/UDP from Risk (orders).
- casting/UDP to Risk (fills/dones).
...
Links:
- casting/UDP or WAL/TCP from Matching (events).
- WAL/TCP replay bootstrap (replication).
