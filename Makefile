...
help:
	...
	@echo "  make gate-4-playwright - Gate 4: Playwright 419/419 (requires gate-3 first)"
	@echo ""
	@echo "Quality:"
	...
	@echo "  make release-gate      - BLOCK release unless Playwright==419/419 and all gates green"
...
#        make gate-4-playwright (full Playwright suite, 419 tests, ~2min)
#
...
# Fails if denominator != 419 or header diverges from log counts.
regen-progress:
...
# Release gate: blocks unless Playwright==419/419 AND all upstream gates green.
...
print(f'[release-gate] playwright={pw}/419 all_green={ok} canonical_ok={canon}'); \
...
# Covers: denominator != 419, phase-semantics zombie state, DONE-FAIL splits.
...
# CI guard: validate artifact JSON — fail on denominator != 419 or
